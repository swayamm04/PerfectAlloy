...
export default function UsersView() {
  ...
  useEffect(() => {
    if (currentUser?.role === "super-admin") {
      fetchUsers();
      if (currentUser?.module !== 'expenses') {
        fetchDepartments();
      }
    }
  }, [currentUser]);
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          role,
          department: currentUser?.module === 'expenses' ? undefined : department
        }),
      });
  ...
  const exportToExcel = () => {
    ...
    const isExpenses = currentUser?.module === 'expenses';
    const headers = isExpenses 
      ? ["Sl No", "User Name", "Email", "Role", "Joined Date"]
      : ["Sl No", "User Name", "Email", "Department", "Role", "Joined Date"];

    const dataRows = filteredUsers.map((u, index) => {
      const row = [
        index + 1,
        u.name,
        u.email,
      ...
      if (!isExpenses) {
        row.push(u.role === 'super-admin' ? "N/A" : (u.department?.name || "N/A"));
      }
      row.push(u.role === 'super-admin' ? "Super Admin" : "User");
      row.push(u.createdAt ? new Date(u.createdAt).toLocaleDateString('en-GB') : "N/A");
      return row;
    });
    ...
    XLSX.writeFile(workbook, `Users_${todayStr}.xlsx`);
          ...
          <Card className="border-primary/20 bg-primary/5 shadow-xl">
            ...
            <CardContent>
              <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {role !== 'super-admin' && currentUser?.module !== 'expenses' && (
                  <div className="space-y-2">
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={currentUser?.module === 'expenses' ? 5 : 6} className="h-40 text-center text-muted-foreground">
