...
export default function ProductionTableView({ id }: ProductionTableViewProps) {
  ...
  const handleAddRow = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/master-tables/${id}/rows`, {
        method: "POST",
        headers: {
  ...
  const exportToExcel = () => {
    if (!table || filteredRows.length === 0) return;
    ...
    const headers = [
      ...
      "Customer",
      "Heat No.",
      ...table.departments.map(d => d.name)
    ];
    ...
    const dataRows = filteredRows.map(row => {
      ...
      table.departments.forEach(dept => {
        ...
        if (deptStages.length === 0) {
          ...
          const cellStr = deptStages.map(stage => {
            if (!stage.inward?.receivedAt) return "Not Received";
            let text = `In: ${stage.inward.qty}`;
            if (stage.outward?.isCompleted) {
              text += ` | Out: ${stage.outward.qty}`;
            } else {
              text += ` | Processing`;
            }
            return text;
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Production Tracker");

    const todayStr = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(workbook, `Production_Table_${table.name.replace(/[^a-zA-Z0-9]/g, "_")}_${todayStr}.xlsx`);
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`PRODUCTION TRACKING - ${table.name.toUpperCase()}`, 15, 22);
    ...
    const todayStr = new Date().toISOString().slice(0, 10);
    doc.save(`Production_Table_${table.name.replace(/[^a-zA-Z0-9]/g, "_")}_${todayStr}.pdf`);
        ...
        <div className="flex items-center gap-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-9 px-4 font-bold border-primary/10 hover:bg-primary/5 transition-colors gap-2 text-xs">
