...
export default function MachineHourRateView() {
  ...
  const canEditSelectedMachine = useMemo(() => {
    if (!currentUser) return false;
    if (currentUser.role === "super-admin") return true;

    // Regular users can only edit if they're in the assignedUsers list for this machine row
    const matchingRow = equipmentRows.find(r => 
      r.designation.toLowerCase().trim() === selectedMachine.toLowerCase().trim()
    );
    if (!matchingRow || !matchingRow.assignedUsers) return false;
    
    return matchingRow.assignedUsers.some((u: any) => 
      String(u._id || u) === String(currentUser._id)
    );
  }, [currentUser, selectedMachine, equipmentRows]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (eqResponse.ok) {
        ...
        eqList = eqData.rows || [];
        eqCols = eqData.columns || [];
      }
      const evaluatedEqRows = evaluateTableRows(eqCols, eqList).map((evalRow: any, idx: number) => ({
        ...evalRow,
        assignedUsers: eqList[idx]?.assignedUsers || []
      }));
      ...
      if (mhrResponse.ok) {
        ...
        setMachineRates(Array.isArray(mhrData) ? mhrData : []);
      } else {
        setMachineRates([]);
      }
  ...
  const activeMachineData = useMemo(() => {
    if (!selectedMachine || !Array.isArray(machineRates)) return null;
  ...
  const exportToExcel = () => {
    if (!activeMachineData || !calculations) return;

    const headers = ["SI No", "Particulars", "Allocation / Detail", "Item Cost", "Cost / Hr (INR)"];
    const excelRows: any[] = [];
    ...
    excelRows.push([2, "Labour", "", "", ""]);
    calculations.labourCosts.forEach((labour, index) => {
      const letter = String.fromCharCode(97 + index);
      excelRows.push([
        "",
        `  ${letter}) ${labour.designation}`,
        activeMachineData.values[labour.allocKey] || "0",
        "",
    ...
    const eqRow = equipmentRows.find(r => r.designation.toLowerCase() === selectedMachine.toLowerCase());
    const power = eqRow ? eqRow.values.power : "";
    excelRows.push([
      3,
      "Power",
      power ? `${power} kW` : "0 kW",
      "",
      calculations.powerCost.toFixed(3)
    ]);
    ...
    excelRows.push([
      ...
      "",
      "",
      calculations.consumablesCost.toFixed(2)
    ]);
    ...
    excelRows.push([
      ...
      "",
      "",
      calculations.maintenanceCost.toFixed(2)
    ]);
    ...
    excelRows.push([
      ...
      "",
      "",
      calculations.rentCost.toFixed(2)
    ]);
    ...
    if (hasWiringCost) {
      excelRows.push([
        ...
        "",
        "",
        calculations.wiringCost.toFixed(2)
      ]);
    }

    calculations.customRows.forEach((row, idx) => {
      const si = (hasWiringCost ? 8 : 7) + idx;
      excelRows.push([
        si,
        row.name,
        row.alloc || "",
    ...
    const titleRow = [`${selectedMachine} Machine Hour Cost - 2025`];
    const worksheet = XLSX.utils.aoa_to_sheet([titleRow, [], headers, ...excelRows]);
    
    worksheet["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 4 } }];

    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Machine Hour Cost");

    const todayStr = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(workbook, `Machine_Hour_Cost_${selectedMachine.replace(/[^a-zA-Z0-9]/g, "_")}_${todayStr}.xlsx`);
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`${selectedMachine.toUpperCase()} MACHINE HOUR COST - 2025`, 15, 22);

    const headers = ["SI No", "Particulars", "Allocation / Detail", "Cost / Hr (INR)"];
    const tableRows: any[] = [];

    tableRows.push(["1", "Depreciation & Interest", "-", calculations.depreciationAndInterestCost.toFixed(2)]);
    tableRows.push(["2", "Labour", "", ""]);
    ...
    tableRows.push(["", "  Labour Subtotal", "", calculations.labourSubtotal.toFixed(2)]);

    const eqRow = equipmentRows.find(r => r.designation.toLowerCase() === selectedMachine.toLowerCase());
    const power = eqRow ? eqRow.values.power : "";
    tableRows.push(["3", "Power", power ? `${power} kW` : "0 kW", calculations.powerCost.toFixed(3)]);
    tableRows.push(["4", "Consumables", "", calculations.consumablesCost.toFixed(2)]);
    tableRows.push(["5", "Maintenance", "", calculations.maintenanceCost.toFixed(2)]);
    tableRows.push(["6", "Rent", "", calculations.rentCost.toFixed(2)]);

    if (hasWiringCost) {
      tableRows.push(["7", "Wiring", "", calculations.wiringCost.toFixed(2)]);
    }
    ...
    tableRows.push(["", "TOTAL Machine Hour Rate", "", calculations.totalMachineHrRate.toFixed(2)]);
    tableRows.push(["", "Utilisation factor", `${calculations.utFactor}%`, ""]);
    tableRows.push(["", "Utilisation Cost", "", calculations.utilisationCost.toFixed(2)]);
    tableRows.push(["", "Cost per second", "", calculations.costPerSecond.toFixed(4)]);
    ...
    const todayStr = new Date().toISOString().slice(0, 10);
    doc.save(`Machine_Hour_Cost_${selectedMachine.replace(/[^a-zA-Z0-9]/g, "_")}_${todayStr}.pdf`);
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMachineRates(Array.isArray(data) ? data : []);
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {isEditing && (
                          <span className="text-[10px] text-primary font-semibold flex items-center gap-1 bg-primary/10 px-2 py-1 rounded-md border border-primary/20 hover:bg-primary/20 transition-colors">
                            Select roles <ChevronDown className="h-2.5 w-2.5" />
                          </span>
                        )}
                      </div>
