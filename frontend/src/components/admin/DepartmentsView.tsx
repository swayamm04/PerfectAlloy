...
export default function DepartmentsView() {
  ...
  const filteredDepartments = departments.filter(d => 
    d.description !== 'Auto-extracted from Final Cost Sheet columns' &&
  ...
  const exportToExcel = () => {
    ...
    const headers = ["Sl No", "Department Name", "Description"];
    const dataRows = filteredDepartments.map((d, index) => [
      index + 1,
      d.name,
    ...
    XLSX.writeFile(workbook, `Departments_${todayStr}.xlsx`);
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text("DEPARTMENTS LIST", 15, 22);
    ...
    const todayStr = new Date().toISOString().slice(0, 10);
    doc.save(`Departments_${todayStr}.pdf`);
